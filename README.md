...
#### 设置页面
![设置页](docs/setting.jpg)

#### 任务列表页面
![任务列表页](docs/task_list.jpg)

#### 下载文件列表
![下载文件列表](docs/file_list.jpg)
...
- **发布方式**: 自动创建 GitHub Release（草稿）并上传所有平台的安装包

**发布步骤**:
```bash
# 1. 确保代码在 main 分支已测试通过
git checkout main
git pull

# 2. 合并到 release 分支并推送（触发自动发布）
...
- **检查项目**: 前端构建、Rust 测试、跨平台构建测试
- **支持平台**: Windows、macOS、Linux

**测试步骤**:
```bash
# 推送到 main 或 develop 分支会自动触发测试
